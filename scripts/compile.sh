    ...
    do
        while IFS=',' read -r pkg file; 
            do
                ...
                solc --evm-version london --abi --bin --optimize --optimize-runs 200 --allow-paths . --include-path node_modules/ --base-path . --overwrite --output-dir "$CW_CONTRACT_OUTPUT_PATH/$pkg/" "$CW_CONTRACT_ACCOUNT_PATH/$file.sol"
    ...
    do
        while IFS=',' read -r pkg file; 
            do
                ...
                mkdir $CW_CONTRACT_OUTPUT_PATH/$pkg
                solc --evm-version london --abi --bin --optimize --optimize-runs 200 --allow-paths . --include-path node_modules/ --base-path . --overwrite --output-dir "$CW_CONTRACT_OUTPUT_PATH/$pkg/" "$CW_CONTRACT_APP_PATH/$file.sol"
