...
class Account extends _i1.GeneratedContract {
  ...
  Future<_i1.EthereumAddress> _authorizer({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[1];
    assert(checkSignature(function, 'a00001f6'));
    final params = [];
  ...
  Future<String> initialize(
    ...
    _i1.EthereumAddress anAuthorizer, {
    required _i1.Credentials credentials,
    _i1.Transaction? transaction,
  }) async {
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i1.EthereumAddress> nonceManager({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[11];
    assert(checkSignature(function, 'fddc205f'));
    final params = [];
    final response = await read(
      function,
      params,
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> updateAuthorizer(
    _i1.EthereumAddress newAuthorizer, {
    required _i1.Credentials credentials,
    _i1.Transaction? transaction,
  }) async {
    final function = self.abi.functions[21];
    assert(checkSignature(function, 'e0a66578'));
    final params = [newAuthorizer];
    return write(
      credentials,
      transaction,
      function,
  ...
  Future<String> updateWhitelist(
    List<_i1.EthereumAddress> newWhitelist, {
    required _i1.Credentials credentials,
    _i1.Transaction? transaction,
  }) async {
    final function = self.abi.functions[22];
    assert(checkSignature(function, '35377214'));
    final params = [newWhitelist];
    return write(
      credentials,
      transaction,
      function,
