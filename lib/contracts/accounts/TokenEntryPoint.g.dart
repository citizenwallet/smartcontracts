...
class TokenEntryPoint extends _i1.GeneratedContract {
  ...
  Future<String> initialize(
    ...
    _i1.EthereumAddress aPaymaster,
    List<_i1.EthereumAddress> addresses, {
    required _i1.Credentials credentials,
    _i1.Transaction? transaction,
  }) async {
    final function = self.abi.functions[5];
    assert(checkSignature(function, '77a24f36'));
    final params = [
      anOwner,
      aPaymaster,
      addresses,
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i1.EthereumAddress> paymaster({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[7];
    assert(checkSignature(function, '16e4cbf9'));
    final params = [];
    final response = await read(
      function,
      params,
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> senderNonce(
    _i1.EthereumAddress $param9, {
    _i1.BlockNum? atBlock,
  }) async {
    final function = self.abi.functions[10];
    assert(checkSignature(function, '9c90b443'));
    final params = [$param9];
    final response = await read(
      function,
      params,
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> updatePaymaster(
    _i1.EthereumAddress newPaymaster, {
    required _i1.Credentials credentials,
    _i1.Transaction? transaction,
  }) async {
    final function = self.abi.functions[12];
    assert(checkSignature(function, 'daf6a3f6'));
    final params = [newPaymaster];
    return write(
      credentials,
      transaction,
      function,
