...
class Authorizer extends _i1.GeneratedContract {
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> updatePaymasterSponsor(
    _i1.EthereumAddress newSponsor, {
    required _i1.Credentials credentials,
    _i1.Transaction? transaction,
  }) async {
    final function = self.abi.functions[12];
    assert(checkSignature(function, 'ba4fdba0'));
  ...
  /// Returns a live stream of all PaymasterSponsorUpdated events emitted by this contract.
  Stream<PaymasterSponsorUpdated> paymasterSponsorUpdatedEvents({
    _i1.BlockNum? fromBlock,
    _i1.BlockNum? toBlock,
  }) {
    final event = self.event('PaymasterSponsorUpdated');
    final filter = _i1.FilterOptions.events(
      contract: self,
      event: event,
      fromBlock: fromBlock,
    ...
    return client.events(filter).map((_i1.FilterEvent result) {
      ...
      return PaymasterSponsorUpdated(
        decoded,
        result,
      );
    });
...
class PaymasterSponsorUpdated {
  PaymasterSponsorUpdated(
    List<dynamic> response,
    this.event,
  ) : newVerifier = (response[0] as _i1.EthereumAddress);

  final _i1.EthereumAddress newVerifier;
