...
class Voucher extends _i1.GeneratedContract {
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> recoverOwnership(
    _i1.EthereumAddress newOwner, {
    required _i1.Credentials credentials,
    _i1.Transaction? transaction,
  }) async {
    final function = self.abi.functions[14];
    assert(checkSignature(function, '0b699abe'));
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i1.EthereumAddress> tokenEntryPoint({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[16];
    assert(checkSignature(function, 'bb503206'));
    final params = [];
    final response = await read(
      function,
      params,
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> validateUserOp$2(
    dynamic userOp,
    _i2.Uint8List userOpHash, {
    required _i1.Credentials credentials,
    _i1.Transaction? transaction,
    ...
    assert(checkSignature(function, 'fff35b72'));
    final params = [
      userOp,
      userOpHash,
    ];
  ...
  /// Returns a live stream of all VoucherInitialized events emitted by this contract.
  Stream<VoucherInitialized> voucherInitializedEvents({
    _i1.BlockNum? fromBlock,
    _i1.BlockNum? toBlock,
  }) {
    final event = self.event('VoucherInitialized');
    final filter = _i1.FilterOptions.events(
      contract: self,
      event: event,
      fromBlock: fromBlock,
