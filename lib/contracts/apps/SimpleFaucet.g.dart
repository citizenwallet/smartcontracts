...
class SimpleFaucet extends _i1.GeneratedContract {
  SimpleFaucet({
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i2.Uint8List> REDEEM_ADMIN_ROLE({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[1];
    assert(checkSignature(function, '5018c209'));
    final params = [];
    final response = await read(
      function,
      params,
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> amount({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[2];
    assert(checkSignature(function, 'aa8c217c'));
    final params = [];
    final response = await read(
      function,
      params,
  ...
  Future<String> initialize(
    ...
    assert(checkSignature(function, '77202ba1'));
    final params = [
      owner,
      _token,
      _amount,
  ...
  Future<String> redeem({
    ...
    assert(checkSignature(function, 'be040fb0'));
    final params = [];
    return write(
      credentials,
      transaction,
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i1.EthereumAddress> redeemAdmin({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[10];
    assert(checkSignature(function, 'a2d2c71c'));
    final params = [];
    final response = await read(
      function,
      params,
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> redeemTo(
    _i1.EthereumAddress to, {
    required _i1.Credentials credentials,
    _i1.Transaction? transaction,
  }) async {
    final function = self.abi.functions[12];
    assert(checkSignature(function, 'f984c55c'));
    final params = [to];
    return write(
      credentials,
      transaction,
  ...
  Future<BigInt> redeemed(
    ...
    assert(checkSignature(function, '9f4568ef'));
